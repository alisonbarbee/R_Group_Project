...
```{r}
# creates the datatable for the Australian weather stations
library(sf)
...
ggplot() + 
  ...
  geom_sf(data = aus_fires, colour = "red", opacity = aus_fires1$brightness) +
  geom_sf(data = aus_stations, colour = "blue", size = 3) + 
  ggtitle("Australia Fires During the Week of Dec. 29th, 2019 - Jan. 5th, 2020") +
  xlab("Longitude") + ylab("Latitiude") +
  geom_text(label = aus_stations$station_name) +
  coord_sf() +
  theme_bw()

# Need to make a function for the colors in order to vary
#add labels on the stations
```
